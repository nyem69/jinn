...
function freshDb(): Database.Database {
  ...
  // Schema is owned by the migration runner. migrateSessionsSchema
  // (compat shim) applies all baseline migrations including sessions,
  // events, handler-seed, and checkpoints, so the explicit
  // initEventsSchema/initHandlerRegistry calls below are no-ops kept
  // only for documentation parity with the original test layout.
